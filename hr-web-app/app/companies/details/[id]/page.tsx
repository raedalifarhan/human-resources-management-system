...
import Headings from "@/app/components/company/Headings";
...
export default function Details({ params }: { params: { id: string } }) {
  ...
  const imageSrc = company.imageUrl && company.imageUrl.startsWith('http')
    ? company.imageUrl
    : `${process.env.NEXT_PUBLIC_API_URL}/images/${company.imageUrl}`;
    ...
    <div className=" flex flex-col gap-10 w-full mx-auto max-w-[90%] rounded-lg shadow-lg p-10 bg-white text-slate-800">
      <div className='grid grid-cols-3 gap-4 '>
        <div>
          ...
          <strong>مسؤول الامتثال</strong>
          <p className="mb-2 ">
            {company.complianceOfficer}
          </p>

          <strong>رقم مسؤول الامتثال</strong>
          <p className="mb-2 ">
            {company.complianceOfficerPhone}
          </p>
        </div>
